...
import { promisifySocket } from '@kindpanda/promisify-socket';
...
class SocketioClient extends EventEmitter {
  ...
  setStatus(status) {
    ...
    if (this.status !== status) {
      ...
      this.logger.debug('Status changed', { previous, to: status });
      this.status = status;
      const eventKey = Object.keys(STATUS).reduce(
        (res, statusKey) => (!res && status === STATUS[statusKey] ? statusKey : res),
        '',
      );
      this.emit(EVENTS[eventKey]);
      this.emit(EVENTS.STATUS_CHANGED, { previous, to: status });
    }
  }
  ...
  initSocket() {
    ...
    this.socket = socketioClient.connect(this.serverAddress, this.socketOptions);
    this.socket = promisifySocket(this.socket);

    this.emit(EVENTS.SOCKET_INITIALIZED);

    [SOCKET_EVENTS.CONNECT, SOCKET_EVENTS.RECONNECT].forEach(event =>
      this.socket.on(event, () => {
        this.logger.info('Socket is connected', { event });
    ...
    [SOCKET_EVENTS.RECONNECT_ERROR, SOCKET_EVENTS.RECONNECT_FAILED, SOCKET_EVENTS.DISCONNECT].forEach(event =>
      this.socket.on(event, () => {
        this.logger.info('Socket is disconnected', { event });
  ...
  async wait(duration = 500) {
    this.logger.debug('wait', { duration });
    return new Promise(resolve => setTimeout(resolve, duration));
  }

  async start({ timeout = this.defaultTimeout } = {}) {
    const options = { timeout };
    const logger = this.logger.context({ action: 'start', options });
  ...
  async stop({ timeout = this.defaultTimeout } = {}) {
    const options = { timeout };
    ...
    this.logger.debug('Disconnect socket', { timeout });
    return Promise.race([
      new Promise(resolve => {
